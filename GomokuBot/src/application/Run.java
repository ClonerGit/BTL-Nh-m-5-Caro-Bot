...
public class Run extends Application {
    ...
    public void start(Stage primaryStage) {
        ...
        Alert chooseDifficulty = new Alert(AlertType.CONFIRMATION);
        chooseDifficulty.setTitle("Chọn độ khó");
        chooseDifficulty.setHeaderText("Hãy chọn độ khó của AI:");
        ButtonType easy = new ButtonType("Dễ");
        ButtonType medium = new ButtonType("Trung bình");
        ButtonType hard = new ButtonType("Khó");
        chooseDifficulty.getButtonTypes().setAll(easy, medium, hard);

        ButtonType difficultyResult = chooseDifficulty.showAndWait().orElse(medium);
    ...
    private int[] getAIMove(Board board) {
        switch (difficulty) {
            case EASY:
                return GomokuAIEasy.getMove(board);
            case MEDIUM:
                return GomokuAIMedium.getMove(board);
            case HARD:
            default:
                return GomokuAIHard.getMove(board);
        }
    }
